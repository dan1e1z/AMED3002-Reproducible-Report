...
author: "Daniel, Muhammad, ZiYi, LingQi, Ruonan"
date: "`r Sys.Date()`"
output:
  html_document:
    theme: flatly
...
```{r fig1, out.height="400px",  echo=FALSE}
...
Figure 1. Workflow of the experimental strategy used to investigate differentially expressed genes between 10 fibrosis (F) and 9 non-fibrosis (NF) breast cancer patient cells, all treated by radiation therapy (Vinasco-Sandoval et al. 2025, Figure 1A).
:::
...
Reproduced the heatmap of differentially expressed lncRNAs distinguishing fibrosis (F) and non-fibrosis (NF) samples using EdgeR-filtered data and log2 CPM normalisation as per the paper ([lncRNA Differential Expression Analysis](#lncrna-differential-expression-analysis)).

Data were normalised with R’s cpm function, and samples ordered by group (NF then F) to match the original. Z-score scaling by gene was applied for clarity, though not specified by the authors ([Heatmap Code](#heatmap)).

The heatmap clearly separated groups, supporting the original findings. Lack of detail on scaling is a minor reproducibility limitation (Figure 3).
...
Differential expression analysis was conducted using EdgeR, applying the criteria of P < 0.05 and absolute fold change ≥ 1.5, to reproduce the volcano plot of lncRNAs distinguishing fibrosis (F) and non-fibrosis (NF) samples. This analysis identified 378 differentially expressed lncRNAs, comprising 234 downregulated and 144 upregulated transcripts, which is fewer than the 531 reported in the original study ([lncRNA Differential Expression Analysis](#lncrna-differential-expression-analysis), [volcano Code](#volcano)).

The discrepancy likely reflects undocumented differences in pre-processing, gene filtering, or software versions that influenced gene inclusion prior to differential analysis. The original study did not provide sufficient methodological detail to enable exact reproduction of these steps.

Nonetheless, the reproduced volcano plot clearly reflected group separation, consistent with the original findings and supporting the conclusion that the noncoding transcriptome discriminates patient groups (Figure 4). However, the omission of key filtering and processing information remains a limitation for reproducibility.
...
::: {style="flex: 1; text-align: center; font-weight: bold;"}
Our Volcano Plots
```{r pathway-specific-volcano, fig.width=10, fig.height=18, out.width='100%'}
...
Differential expression analysis of protein-coding genes was performed using EdgeR, applying the same criteria as the original study (P < 0.05, |fold change| ≥ 1.5). This yielded 2,162 differentially expressed mRNAs, consistent with the reported total.

Functional enrichment analysis using ReactomePA identified significantly enriched pathways, including TGFB signalling, extracellular matrix organisation, and collagen synthesis (FDR < 0.05). To reproduce the pathway-specific volcano plots (Figure 5A–C, left), genes were filtered to include only those within each Reactome pathway and meeting the differential expression thresholds ([Pathway Enrichment Preparation](#pathway-enrichment-preparation), [Pathway Volcano Code](#pathway-specific-volcano)).

While the plots were successfully generated using this approach, exact reproduction was limited by insufficient detail on the pathway filtering and gene-to-pathway mapping procedures in the original methods. Specifically, the paper did not describe the Reactome version used or the precise logic for subsetting DE genes for these plots.

Despite these gaps, the reproduced plots (Figure 5, left) captured key pathway-level expression differences between fibrosis and non-fibrosis samples, supporting the original study’s conclusions as visualised in their corresponding plots (Vinasco-Sandoval et al. 2025, Figure 1C–E; Figure 5, right).